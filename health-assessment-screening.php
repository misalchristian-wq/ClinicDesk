...
<head>
  ...
  <style>
    /* Keep your existing CSS – it's already good */
    :root {
    ...
    * { box-sizing: border-box; }
    ...
    .header-icon {
      ...
      font-size: 30px; margin-right: 16px;
    ...
    .btn-back {
      ...
      font-weight: 800; box-shadow: 0 12px 28px rgba(0,0,0,0.12);
    ...
    .card-box {
      ...
      border-radius: var(--clinic-radius); box-shadow: var(--clinic-shadow);
      padding: 24px;
    }
    ...
    .form-control, .form-select {
      border-radius: 14px; border: 1px solid var(--clinic-border);
      padding: 11px 13px; font-size: 14px; background: white;
    }
    .form-control:focus, .form-select:focus {
      border-color: var(--clinic-secondary); box-shadow: 0 0 0 0.2rem rgba(20,184,166,0.16);
    }
    .btn-green {
      background: linear-gradient(135deg, var(--clinic-primary), var(--clinic-secondary));
      color: white; font-weight: 900; border: none; border-radius: 14px;
      padding: 11px 16px; box-shadow: 0 12px 24px rgba(15,118,110,0.18);
    }
    .btn-green:hover { color: white; transform: translateY(-1px); }
    .btn-outline-clinic {
      border: 1px solid var(--clinic-primary); color: var(--clinic-primary);
      background: white; font-weight: 900; border-radius: 14px; padding: 10px 14px;
    }
    .btn-outline-clinic:hover { background: var(--clinic-primary); color: white; }

    .symptoms-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px,1fr)); gap: 12px; margin-top: 8px; }
...
<div id="app" class="wrapper">
  ...
  <div class="header-box d-flex justify-content-between align-items-center flex-wrap gap-3">
    <div class="header-content d-flex align-items-center">
      ...
      <div>
        ...
        <p class="mb-1">Update the learner’s clinical history, symptoms, and follow‑up plan.</p>
  ...
  <div class="main-grid">
    ...
    <!-- Right content – health assessment form -->
    <div>
      ...
      <!-- Optional: Meal Plan Calendar (static for now) -->
      <div class="card-box">
        <div class="section-title">
          <h3>Meal Plan Calendar <span class="small-note">(Demo – not saved)</span></h3>
          <p>Recommended meals for each day. The clinic nurse can mark progress daily (frontend only).</p>
        </div>
        <div class="row">
          <div class="col-md-6 col-lg-3 mb-3" v-for="(day, idx) in mealPlan" :key="idx">
            <div class="border rounded p-2 h-100">
              ...
              <button class="btn btn-sm btn-outline-secondary w-100" @click="day.completed = !day.completed">
                {{ day.completed ? '✓ Completed' : 'Mark Completed' }}
              </button>
            </div>
          </div>
        </div>
        <p class="small-note mt-2 mb-0 text-center"><em>Meal plan completion is not yet saved to the database – this is a visual tool.</em></p>
      </div>
    </div>
  </div>
</div>
...
createApp({
  data() {
    return {
      ...
      mealPlan: [
        { day: 'Monday', breakfast: 'Egg, rice, banana', lunch: 'Chicken, vegetables, rice', snack: 'Milk, bread', dinner: 'Fish, soup, rice', completed: false },
        { day: 'Tuesday', breakfast: 'Oatmeal, milk, fruit', lunch: 'Egg, monggo, rice', snack: 'Banana, sandwich', dinner: 'Chicken soup, rice', completed: false },
        { day: 'Wednesday', breakfast: 'Rice, egg, fruit', lunch: 'Fish, vegetables, rice', snack: 'Milk, crackers', dinner: 'Vegetable soup, rice', completed: false },
        { day: 'Thursday', breakfast: 'Bread, peanut butter, milk', lunch: 'Chicken, squash, rice', snack: 'Fruit, yogurt', dinner: 'Egg, vegetables, rice', completed: false },
        { day: 'Friday', breakfast: 'Oatmeal, banana, milk', lunch: 'Fish, monggo, rice', snack: 'Sandwich', dinner: 'Chicken, vegetables, rice', completed: false },
        { day: 'Saturday', breakfast: 'Egg, rice, fruit', lunch: 'Vegetable soup, rice', snack: 'Milk, bread', dinner: 'Fish, rice, vegetables', completed: false },
        { day: 'Sunday', breakfast: 'Rice, egg, milk', lunch: 'Chicken, vegetables, rice', snack: 'Fruit', dinner: 'Soup, rice, fish', completed: false }
      ]
    };
  },
  ...
  methods: {
        ...
        if (data.success && data.health_input) {
          ...
          Object.keys(this.form).forEach(key => {
            if (data.health_input.hasOwnProperty(key)) {
              // Convert 'Yes'/'No' or 1/0 to boolean where appropriate
    ...
    async saveHealthAssessment() {
      ...
      // Convert boolean to "Yes"/"No" for backend compatibility (backend accepts Yes/No or 0/1)
